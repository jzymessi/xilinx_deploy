...
class Classification : public vitis::ai::ConfigurableDpuTaskBase {
 public:
  ...
  virtual std::vector<std::vector<int>> run(const vector<vector<char>>& input_chars) = 0;
};
...
private:
  virtual std::vector<std::vector<int>> run(const vector<vector<char>>& input_chars) override;
private:
  int preprocess_type;
  const int TOP_K;
...
extern std::vector<std::vector<int>> run_xmodel(std::vector<std::shared_ptr<Classification>>& models, std::vector<std::string> vectorLines, int num_of_threads);
